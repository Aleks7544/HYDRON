...
{
    public interface IValidatorRankingService
    {
        List<ValidatorRank> Rank(
            IEnumerable<Models.Validator> allValidators,
            int coreCapacity,
    ...
    public class ValidatorRankingService : IValidatorRankingService
    {
        ...
        public List<ValidatorRank> Rank(
            IEnumerable<Models.Validator> allValidators,
            int coreCapacity,
            IEnumerable<Validation> validationsFromLastNBlocks,
            int blocksObserved)
        {
            List<Models.Validator> validators = allValidators.ToList();
            ...
            List<ValidatorRank> ranked = scores
                .OrderByDescending(s => s.FinalRank)
                .ToList();
            ...
            for (int i = 0; i < ranked.Count; i++)
            {
                ...
                {
                    Tier = i < coreCapacity ? ValidatorTier.Core : ValidatorTier.Edge
                };
            }
        ...
        private static Dictionary<string, double> ComputeAvgSpeedPerValidator(
            ...
            validations
                ...
                .ToDictionary(g => g.Key, g => g.Average(v => v.ValidationSpeedMs!.Value));
        ...
        private static double[] Normalize(List<double> values)
        {
            ...
            return range == 0
                ...
                : values.Select(v => (v - min) / range).ToArray();
        }
        ...
        private static double[] NormalizeInverted(List<double> values)
        {
            ...
            return range == 0
                ? values.Select(_ => 1.0).ToArray()
                : values.Select(v => 1.0 - (v - min) / range).ToArray();
        }
    }
}