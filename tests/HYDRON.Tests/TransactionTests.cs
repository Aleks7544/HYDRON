...
public class TransactionTests
{
    ...
    public void UpdateStatus_InvalidTransition_Throws()
        => Assert.Throws<InvalidOperationException>(() =>
            MakeTx().UpdateStatus(TransactionStatus.Settled));
    ...
    public void FinalizeTransaction_FromNonTerminalStatus_Throws()
        => Assert.Throws<InvalidOperationException>(() => MakeTx().FinalizeTransaction());
    ...
    public void GetTotalCost_ReturnsSumOfAmountAndFee()
        => Assert.Equal(new Atomos(1100), MakeTx(amount: new Atomos(1000)).GetTotalCost());
    ...
    public void AssignBlockNumber_BeforeSettled_Throws()
        => Assert.Throws<InvalidOperationException>(() => MakeTx().AssignBlockNumber(BigInteger.One));
}
